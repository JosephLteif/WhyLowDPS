...
const getEffectiveTier = (item: ExternalItem, selectedDifficulty: string, itemTiers: Record<number, number>, upgradeTracks: Record<string, { level: number; ilvl: number }[]>) => {
  const info = item.difficulty_info?.[selectedDifficulty] || item.dungeon_info?.[selectedDifficulty];
...
interface AddItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (item: ExternalItem, difficulty: string, overrides?: { bonus_ids: number[]; ilvl: number; track_name: string; level: number }) => void;
...
export default function AddItemModal({ isOpen, onClose, onAdd, className, spec, preferredSlot }: AddItemModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchDrops = async () => {
      ...
      try {
        ...
        if (selectedInstance === 0) { // All Sources
  ...
  const filteredInstances = useMemo(() => {
    ...
    if (selectedInstance === -2) {
      base = [
        { id: -2, name: "Search Results", type: "search" },
        ...instances
      ];
    } else {
      base = instances.filter(inst => {
        ...
        if (category === 'dungeon') return type === 'dungeon' || type === 'expansion-dungeon' || type === 'mplus-chest';
  ...
  const globalSearchResults = useMemo(() => {
    ...
    for (const items of Object.values(allPossibleDrops)) {
      for (const item of items) {
        ...
        const matchesSource = item.encounter.toLowerCase().includes(lowerQuery) || item.instance_name.toLowerCase().includes(lowerQuery);
  ...
  const filteredDrops = useMemo(() => {
    ...
    const sourceData = selectedInstance === -2 
  ...
  const difficulties = useMemo(() => {
    ...
    if (instance.type === 'raid') {
      ...
      let group = seasonConfig.dungeon_categories.find(c => 
  ...
  const handleAdd = (item: ExternalItem) => {
    ...
    onAdd(item, selectedDifficulty, info ? { 
  ...
  const getEffectiveTierLocal = (item: ExternalItem) => getEffectiveTier(item, selectedDifficulty, itemTiers, upgradeTracks);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={(e) => {
      ...
      <div className="relative flex h-[85vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0a0a0c] shadow-2xl animate-in fade-in zoom-in duration-200">
        {/* Header */}
        <div className="border-b border-white/5 bg-white/[0.02]">
          ...
          <div className="flex items-center justify-between px-6 py-3 border-b border-white/5">
            <div className="flex items-center gap-6">
              <div className="shrink-0">
                <h2 className="text-lg font-bold bg-gradient-to-br from-white to-white/60 bg-clip-text text-transparent">
                  External Gear <span className="text-[10px] text-blue-500/50 align-top ml-1">v1.2</span>
                </h2>
                <p className="text-[9px] uppercase tracking-widest font-bold text-slate-600 mt-0.5">Loot Browser</p>
              </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const firstInCat = instances.find(inst => {
                        ...
                        if (cat === 'dungeon') return type === 'dungeon' || type === 'expansion-dungeon' || type === 'mplus-chest';
                        return !type.includes('pvp') && !type.includes('profession');
                      });
            ...
            <button
              onClick={onClose}
              className="group relative flex items-center justify-center h-8 w-8 rounded-xl bg-white/[0.03] border border-white/5 text-slate-500 hover:text-white hover:bg-red-500/10 hover:border-red-500/20 transition-all duration-200"
              title="Close Modal"
            >
          ...
          <div className="px-6 py-2.5 bg-black/10 flex items-center gap-6 relative">
            <div className="relative flex-1 group" onFocus={() => setShowSearchDropdown(true)}>
              <input
                type="text"
                ...
                className="w-full bg-black/40 border border-white/5 rounded-xl px-10 py-2 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-500/50 transition-all font-medium"
              />
              <svg className="absolute left-3.5 top-2.5 h-3.5 w-3.5 text-slate-600 group-focus-within:text-blue-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>

              {/* Dropdown Results */}
              {showSearchDropdown && globalSearch.length >= 2 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-[#0d0d10] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200" onClick={(e) => e.stopPropagation()}>
                  <div className="max-h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10">
                    {isGlobalLoading ? (
                      <div className="p-4 text-center text-slate-500 text-[10px] animate-pulse">Syncing Global Loot...</div>
                    ) : globalSearchResults.length === 0 ? (
                      <div className="p-4 text-center text-slate-500 text-[10px]">No matches found for &quot;{globalSearch}&quot;</div>
                    ) : (
                      <>
                        {globalSearchResults.slice(0, 10).map((item) => (
                          <button
                            ...
                            className="w-full flex items-center gap-3 p-3 hover:bg-white/[0.03] transition-colors border-b border-white/5 last:border-0 group"
                          >
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={item.icon || '/assets/unknown.png'} className="w-8 h-8 rounded-lg shadow-lg" alt="" />
                            <div className="text-left flex-1 min-w-0">
                              <div className="text-xs font-bold text-white truncate group-hover:text-blue-400 transition-colors">{item.name}</div>
                              <div className="text-[10px] text-slate-500 truncate">{item.encounter} • {item.instance_name}</div>
                            </div>
                        ...
                        {globalSearchResults.length > 10 && (
                          <div className="p-2 bg-black/40 text-center text-[10px] text-slate-600 italic border-b border-white/5">
                        ...
                        <button
                          ...
                          className="w-full p-3 bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 text-xs font-black uppercase tracking-widest transition-all"
                        >
            ...
            <div className="flex items-center gap-3 shrink-0">
              <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest hidden xl:block">Difficulty</span>
              <div className="flex bg-black/40 rounded-xl p-0.5 border border-white/5 gap-0.5">
                {difficulties.map(d => {
                  const isSelected = selectedDifficulty === d.key;
                  const colorClass = 
                    d.key.includes('lfr') ? 'text-green-400' :
                    d.key.includes('normal') ? 'text-blue-400' :
                    d.key.includes('heroic') ? 'text-purple-400' :
                    d.key.includes('mythic') ? 'text-orange-400' : 'text-slate-400';
                  
                  const activeBg = 
                    d.key.includes('lfr') ? 'bg-green-500/20 border-green-500/20' :
                    d.key.includes('normal') ? 'bg-blue-500/20 border-blue-500/20' :
                    d.key.includes('heroic') ? 'bg-purple-500/20 border-purple-500/20' :
                    d.key.includes('mythic') ? 'bg-orange-500/20 border-orange-500/20' : 'bg-white/10 border-white/10';
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all border ${
                        isSelected ? `${activeBg} ${colorClass} border-white/10` : 'text-slate-600 hover:text-slate-400 border-transparent hover:bg-white/5'
                      }`}
                    >
                      {d.label}
                    </button>
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Sidebar - Instances */}
          <div className="w-72 border-r border-white/5 overflow-y-auto bg-black/20 p-2 space-y-1 scrollbar-thin scrollbar-thumb-white/10">
              ...
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl flex items-center gap-3 transition-all duration-200 group ${
                  selectedInstance === inst.id 
                    ? 'bg-blue-600/10 border border-blue-500/20 text-blue-400 shadow-lg shadow-blue-900/5' 
                    : 'hover:bg-white/[0.03] border border-transparent text-slate-400'
                }`}
              >
                <div className={`w-1.5 h-1.5 rounded-full transition-transform group-hover:scale-125 ${
                  inst.type === 'raid' ? 'bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.4)]' : 
                  inst.type.includes('pvp') ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.4)]' :
          ...
          {/* Main Area - Drops */}
          <div className="flex-1 overflow-y-auto p-6 bg-[#0a0a0c] scrollbar-thin scrollbar-thumb-white/10">
            {loading ? (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <span className="text-slate-500 text-xs font-bold uppercase tracking-widest animate-pulse">Scanning Loot Tables</span>
              </div>
              ...
              <div className="flex flex-col items-center justify-center h-full text-slate-600 gap-4 opacity-50">
                ...
                <div className="text-center">
                  <p className="text-sm font-bold">{selectedInstance === -2 ? `No results for "${globalSearch}"` : 'No Loot Found'}</p>
                  <button onClick={() => { 
                    setGlobalSearch(''); 
                    setLocalSearch(''); 
                    setCategory('raid'); 
                    if (selectedInstance === -2) setSelectedInstance(0);
                  }} className="text-blue-500 hover:text-blue-400 text-xs font-bold mt-2">
              ...
              <div className="space-y-8 pb-12">
                ...
                <div className="relative mb-6">
                  <input
                    ...
                    className="w-full bg-white/[0.03] border border-white/5 rounded-xl px-10 py-3 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:bg-white/[0.05] transition-all"
                  />
                  <svg className="absolute left-4 top-3.5 h-4 w-4 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  {Object.entries(filteredDrops).map(([slot, items]) => {
                   const renderItem = (item: ExternalItem) => {
                     ...
                     const bonusId = tier?.bonus_id || (item.difficulty_info?.[selectedDifficulty] || item.dungeon_info?.[selectedDifficulty])?.bonus_id;
                    ...
                    return (
                      <div
                        ...
                        className="group relative bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 hover:border-white/10 rounded-xl p-3 px-4 transition-all duration-300 hover:shadow-2xl hover:shadow-blue-500/5 hover:-translate-y-0.5"
                      >
                        <div className="flex items-center gap-4">
                          ...
                          >
                            <img
                              ...
                              alt=""
                              className={`w-10 h-10 rounded-lg border-2 shadow-lg transition-transform group-hover:scale-105 ${
                                item.quality === 5 ? 'border-orange-500/50' : 
                                item.quality === 4 ? 'border-purple-500/50' : 
                                item.quality === 3 ? 'border-blue-500/50' : 'border-white/10'
                              }`}
                            />
                            <div className="absolute -top-1.5 -right-1.5 bg-black border border-white/10 px-1 py-0.5 rounded text-[8px] font-black text-blue-400 shadow-xl">
                              {currentIlvl}
                            </div>
                          </a>
                          ...
                          <div className="flex-1 min-w-0 pr-12">
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className={`text-xs font-bold truncate transition-colors block hover:underline ${
                                item.quality === 5 ? 'text-orange-400' : 
                                item.quality === 4 ? 'text-purple-400' : 
                                item.quality === 3 ? 'text-blue-400' : 'text-slate-200'
                              }`}
                              onClick={(e) => e.preventDefault()}
                            >
                              {item.name}
                            </a>
                            <p className="text-[9px] text-slate-500 font-medium truncate mt-0.5 uppercase tracking-tighter">
                              {item.encounter}
                            </p>
                          </div>
                          ...
                          <button
                            ...
                            className="absolute right-3 top-3.5 bg-blue-600 hover:bg-blue-500 text-white p-2 rounded-lg transition-all shadow-lg active:scale-95 group/btn"
                        ...
                        {tier && (
                          <div className="mt-3 pt-3 border-t border-white/5 flex items-center gap-4">
                            <div className="flex flex-col w-20 shrink-0">
                              <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest truncate">{tier.track}</span>
                              <span className="text-[9px] font-mono font-bold text-blue-400">{tier.level} / {tier.maxLevel}</span>
                            </div>
                            <input
                              type="range"
                              min="1"
                              ...
                              className="flex-1 h-1 bg-white/5 rounded-full appearance-none cursor-pointer accent-blue-500 hover:accent-blue-400"
                            />
                          </div>
                        )}
                      </div>
