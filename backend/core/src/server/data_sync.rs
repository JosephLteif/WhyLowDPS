...
pub async fn get_sync_status(
    ...
    if status == SyncStatus::NeedsCredentials {
        if get_effective_credentials(&req, &auth_state, store.get_ref()).is_some() {
            can_sync = true;
        }
    }
...
pub async fn trigger_sync(
    ...
    if creds.is_none() {
        *status = SyncStatus::NeedsCredentials;
        return HttpResponse::BadRequest().json(json!({"detail": "Blizzard API credentials required"}));
    }
    ...
    *status = SyncStatus::Syncing;
    ...
    tokio::spawn(async move {
        if let Err(e) = perform_sync(state_clone.clone(), blizzard_clone, client_id, client_secret, data_dir_clone).await {
...
async fn perform_sync(
    ...
    let res = blizzard.client.get(&metadata_url).send().await.map_err(|e| format!("Failed to fetch metadata: {}", e))?;
    let metadata_text = res.text().await.map_err(|e| format!("Failed to read metadata: {}", e))?;
    ...
    let files: Vec<String> = re.captures_iter(&metadata_text)
    ...
    if let Some(dir) = &data_dir {
        ...
        for (i, file_name) in files.iter().enumerate() {
            ...
            let file_res = blizzard.client.get(&file_url).send().await.map_err(|e| format!("Failed to download {}: {}", file_name, e))?;
            let content = file_res.bytes().await.map_err(|e| format!("Failed to read {}: {}", file_name, e))?;
            
            std::fs::write(&file_path, content).map_err(|e| format!("Failed to save {}: {}", file_name, e))?;
        }
    }
    ...
    let res = blizzard.client.get(season_index_url).bearer_auth(&token).send().await.map_err(|e| e.to_string())?;
    let season_index: Value = res.json().await.map_err(|e| e.to_string())?;
    
    let current_season_id = season_index.get("current_season").and_then(|s| s.get("id")).and_then(|id| id.as_i64())
    ...
    let res = blizzard.client.get(&season_url).bearer_auth(&token).send().await.map_err(|e| e.to_string())?;
    ...
    if let Some(dungeons) = dungeons {
        for (i, dungeon) in dungeons.iter().enumerate() {
            let name = dungeon.get("name").and_then(|n| n.as_str()).unwrap_or("Unknown Dungeon");
            {
                let mut p = state.progress.lock().await;
    ...
    if let Some(dir) = data_dir {
        ...
        std::fs::write(&runtime_file, serde_json::to_string_pretty(&runtime_data).unwrap())
