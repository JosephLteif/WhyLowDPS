...
pub fn difficulty_track_name(difficulty: &str) -> Option<String> {
    ...
    for diff in raid_diffs {
        if diff.get("name").and_then(|n| n.as_str()) == Some(difficulty) {
            return diff.get("track").and_then(|t| t.as_str()).map(|s| s.to_string());
        }
    }
    None
}
    ...
    for &bid in bonus_ids {
        if let Some(bonus) = bonuses.get(&bid) {
            if let Some(upgrade) = &bonus.upgrade {
                if let (Some(full_name), Some(group), Some(level)) = (&upgrade.full_name, upgrade.group, upgrade.level) {
                ...
                for bid in bonuses.keys() {
                    if let Some(b) = bonuses.get(bid) {
                        if let Some(u) = &b.upgrade {
                            if u.group == Some(group_id) && u.level.is_some_and(|lvl| lvl > current_level && lvl <= l) {
...
pub fn get_upgrade_cost_between(from_bonus_id: u64, to_bonus_id: u64) -> HashMap<u64, u64> {
    ...
    if let Some(bonus) = bonuses.get(&from_bonus_id) {
        if let Some(u) = &bonus.upgrade {
            if let (Some(g), Some(l)) = (u.group, u.level) { from_info = Some((g, l)); }
        }
    }
    if let Some(bonus) = bonuses.get(&to_bonus_id) {
        if let Some(u) = &bonus.upgrade {
            if let (Some(g), Some(l)) = (u.group, u.level) { to_info = Some((g, l)); }
        }
    }
    ...
    for bid in bonuses.keys() {
        if let Some(bonus) = bonuses.get(bid) {
            if let Some(u) = &bonus.upgrade {
                if u.group == Some(group) && u.level.is_some_and(|l| l > start_level && l <= end_level) {
                    if let Some(step_cost) = costs_map.get(bid) {
                        for (&cid, &amt) in step_cost {
...
pub fn upgrade_simc_input(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        if let Some(idx) = line.find(",bonus_id=") {
            ...
            let bids: Vec<u64> = bonus_str.split('/').filter_map(|s| s.parse().ok()).collect();
            let upgraded = upgrade_bonus_ids_to_max(&bids);
            let upgraded_str = upgraded.iter().map(|b| b.to_string()).collect::<Vec<_>>().join("/");
...
pub fn upgrade_items_by_slot(mut items: HashMap<String, Vec<ResolvedItem>>) -> HashMap<String, Vec<ResolvedItem>> {
    for list in items.values_mut() {
        for item in list {
            ...
            if upgraded_bids != item.bonus_ids {
                ...
                parts.push(format!("bonus_id={}", item.bonus_ids.iter().map(|b| b.to_string()).collect::<Vec<_>>().join("/")));
                if item.enchant_id > 0 { parts.push(format!("enchant_id={}", item.enchant_id)); }
                if item.gem_id > 0 { parts.push(format!("gem_id={}", item.gem_id)); }
