...
function RouteSelectorModal({
  ...
  const routeCards = useMemo(() => {
    return routes.map((route) => {
      ...
      const timerSeconds =
        route.timer_seconds || (dungeonInfo?.maxTime ? Number(dungeonInfo.maxTime) : 0);
  ...
  const formatHealthCompact = (hp: number) => {
  ...
  const dungeonOptions = useMemo(() => {
    return Array.from(new Set(routeCards.map((c) => c.route.dungeon).filter(Boolean))).sort((a, b) =>
      a.localeCompare(b),
    );
  }, [routeCards]);
                  ...
                  >
                    ...
                    <div className="mt-2 text-[10px] text-zinc-500">
                      ...
                      <span className="font-semibold text-zinc-300">
                        {formatHealthCompact(card.totalHp)}
                      </span>
