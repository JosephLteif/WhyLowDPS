...
export default function TopGearItemSelector({
  ...
  useEffect(() => {
    ...
    if (!el) return;
    const obs = new IntersectionObserver(([entry]) => setHeaderVisible(entry.isIntersecting), { threshold: 0 });
    obs.observe(el);
  ...
  const convertToCatalyst = useCallback(async (item: ResolvedItem) => {
    ...
    try {
      ...
      const nextSelected = { ...Object.fromEntries(Object.entries(selectedUids).map(([k, v]) => [k, new Set(v)])) };
  ...
  const addUpgradedCopy = useCallback((item: ResolvedItem, option: any) => {
    const currentUpgradeBonusId = upgradeOptions.find((o) => item.bonus_ids.includes(o.bonus_id))?.bonus_id;
    if (!currentUpgradeBonusId) return;

    const newBonusIds = item.bonus_ids.map((b) => (b === currentUpgradeBonusId ? option.bonus_id : b));
    const newSimcString = item.simc_string.replace(/bonus_id=[0-9/:]+/, `bonus_id=${newBonusIds.join('/')}`);
    const copyOrigin = 'bags';
    const copy: ResolvedItem = {
      ...item,
      origin: copyOrigin as any,
      uid: makeUid({ item_id: item.item_id, bonus_ids: newBonusIds, origin: copyOrigin, slot: item.slot, enchant_id: item.enchant_id, gem_id: item.gem_id }),
    ...
    onSelectionChange(nextSelected);
    setUpgradeMenuFor(null);
  }, [resolved, upgradeOptions, onResolvedChange, onItemAdded, selectedUids, onSelectionChange, setUpgradeMenuFor]);

  const handleOptimize = useCallback((enchantId: number, gemIds: number[]) => {
    if (!optimizeItem) return;
    const item = optimizeItem;
    const firstGemId = gemIds[0] || 0;
    let nextSimc = item.simc_string;
    if (enchantId > 0) {
      if (nextSimc.includes('enchant_id=')) nextSimc = nextSimc.replace(/enchant_id=[0-9]+/, `enchant_id=${enchantId}`);
      else nextSimc += `,enchant_id=${enchantId}`;
    } else nextSimc = nextSimc.replace(/,enchant_id=[0-9]+/, '');

    if (firstGemId > 0) {
      if (nextSimc.includes('gem_id=')) nextSimc = nextSimc.replace(/gem_id=[0-9/:]+/, `gem_id=${firstGemId}`);
      else nextSimc += `,gem_id=${firstGemId}`;
    } else nextSimc = nextSimc.replace(/,gem_id=[0-9/:]+/, '');

    const uid = makeUid({ item_id: item.item_id, bonus_ids: item.bonus_ids, origin: 'bags', slot: item.slot, enchant_id: enchantId, gem_id: firstGemId });
    const copy: ResolvedItem = { ...item, origin: 'bags', uid, enchant_id: enchantId, gem_id: firstGemId, simc_string: nextSimc, tag: 'Opt' };
  ...
  }, [optimizeItem, resolved, onResolvedChange, onItemAdded, selectedUids, onSelectionChange, setOptimizeOpen]);

  const handleAddItem = useCallback(async (item: any, difficulty: string, overrides?: any) => {
    ...
    const slot = slots[0];
    let bonusIds = overrides ? [...overrides.bonus_ids] : item.difficulty_info?.[difficulty]?.bonus_id ? [item.difficulty_info[difficulty].bonus_id] : [];
    let ilvl = overrides ? overrides.ilvl : item.difficulty_info?.[difficulty]?.ilvl || item.ilevel;
    let upgradeStr = overrides ? `${overrides.track_name} ${overrides.level}` : item.difficulty_info?.[difficulty]?.track ? `${item.difficulty_info[difficulty].track} ${item.difficulty_info[difficulty].level}/${item.difficulty_info[difficulty].max_level}` : '';

    const uid = makeUid({ item_id: item.item_id, bonus_ids: bonusIds, origin: 'bags', slot });
    const newItem: ResolvedItem = {
      uid, slot, item_id: item.item_id, ilevel: ilvl,
      simc_string: `,id=${item.item_id}${ilvl > 0 ? `,ilevel=${ilvl}` : ''}${bonusIds.length > 0 ? `,bonus_id=${bonusIds.join('/')}` : ''},name=${item.name.replace(/ /g, '_')}`,
      origin: 'bags', bonus_ids: bonusIds, enchant_id: 0, gem_id: 0, name: item.name, icon: item.icon, quality: item.quality,
      quality_color: item.quality === 5 ? '#ff8000' : item.quality === 4 ? '#a335ee' : item.quality === 3 ? '#0070dd' : '#1eff00',
      tag: 'Bags', upgrade: upgradeStr, sockets: 0, enchant_name: '', gem_name: '', gem_icon: ''
    };
    ...
    slots.forEach(s => {
      ...
      if (targetSlot && !targetSlot.alternatives.find(a => a.uid === uid)) {
        const slotUid = makeUid({ item_id: item.item_id, bonus_ids: bonusIds, origin: 'bags', slot: s });
    ...
    return DISPLAY_GROUPS.map(group => {
      ...
      group.slots.forEach(slot => {
        ...
        slotRes.alternatives.forEach(alt => {
          ...
          if (!seenAltKeys.has(key)) { seenAltKeys.add(key); alternatives.push(alt); }
        });
      });
  ...
  const itemDetails = (item: ResolvedItem) => {
    ...
    else if (item.sockets > 0) parts.push({ text: `${item.sockets} Socket${item.sockets > 1 ? 's' : ''}`, color: 'text-sky-400/70' });
  ...
  const quickSelect = (
    <TopGearQuickSelect
      ...
      allVaultSelected={vaultUids.length > 0 && vaultUids.every(c => selectedUids[c.slot]?.has(c.uid))}
      catalystCount={catalystUids.length}
      allCatalystSelected={catalystUids.length > 0 && catalystUids.every(c => selectedUids[c.slot]?.has(c.uid))}
  ...
  return (
    <div className="space-y-4">
      <AddItemModal isOpen={isAddItemOpen} onClose={() => setAddItemOpen(false)} onAdd={handleAddItem} className={resolved.character.class_name} spec={resolved.character.spec} preferredSlot={addItemSlot} />
      <OptimizeItemModal isOpen={isOptimizeOpen} onClose={() => setOptimizeOpen(false)} item={optimizeItem} onApply={handleOptimize} />

      {!headerVisible && (
        <div className="fixed left-0 right-0 top-12 z-50 flex items-center justify-between border-b border-border/50 bg-surface/90 px-4 py-2 backdrop-blur-sm shadow-lg">
          <div className="flex items-center gap-3">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted">Items</p>
            <button onClick={() => openAddItem()} className="flex items-center gap-1.5 rounded-md bg-gold/10 px-2 py-1 text-[10px] font-bold tracking-wider text-gold hover:bg-gold/20">Add</button>
          </div>
          {quickSelect}
        </div>
      )}
      ...
      <div ref={headerRef} className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-muted">Select Items</h2>
          <button onClick={() => openAddItem()} className="flex items-center gap-1.5 rounded-md bg-gold/10 px-2.5 py-1.5 text-[10px] font-bold tracking-wider text-gold hover:bg-gold/20">Add Item</button>
        </div>
        {quickSelect}
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {visibleGroups.map(({ group, equipped, alternatives }) => (
          <TopGearSlotGroup
            key={group.label} label={group.label} slots={group.slots} equipped={equipped} alternatives={alternatives}
            selectedUids={selectedUids} upgradeMenuFor={upgradeMenuFor} upgradeOptions={upgradeOptions} loadingUpgrades={loadingUpgrades}
            onToggle={(item) => toggleItem(item, group.slots)} onAddClick={openAddItem} onUpgradeClick={openUpgradeMenu}
            onUpgradeSelect={addUpgradedCopy} onCatalystConvert={convertToCatalyst} onOptimize={openOptimize}
            itemDetails={itemDetails} isItemSelected={(item) => group.slots.some(s => {
            ...
            getWowheadUrl={getWowheadUrl} getWowheadData={getWowheadData}
          />
        ))}
      </div>
    </div>
