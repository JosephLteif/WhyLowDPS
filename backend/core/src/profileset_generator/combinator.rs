...
pub fn build_slot_candidates(base_profile: &str, items_by_slot: &HashMap<String, Vec<ResolvedItem>>, selected_items: &HashMap<String, Vec<String>>) -> HashMap<String, Vec<ResolvedItem>> {
    ...
    for slot in GEAR_SLOTS {
        ...
        if let Some(eq) = slot_items.iter().find(|it| it.origin == ItemOrigin::Equipped) {
            if !candidates.iter().any(|c| c.item_id == eq.item_id && c.origin == ItemOrigin::Equipped) {
...
pub fn filter_valid_combos(all_combos: &[Vec<usize>], varying_slots: &[String], option_lists: &[&Vec<ResolvedItem>], slot_item_lists: &HashMap<String, Vec<ResolvedItem>>, spec: &str, catalyst_charges: Option<u32>) -> Vec<HashMap<String, ResolvedItem>> {
...
pub fn build_gear_set_from_combo(indices: &[usize], varying_slots: &[String], option_lists: &[&Vec<ResolvedItem>], slot_item_lists: &HashMap<String, Vec<ResolvedItem>>, spec: &str) -> HashMap<String, ResolvedItem> {
    let mut gear_set = HashMap::new();
    for slot in GEAR_SLOTS {
        let s_str = slot.to_string();
        if let Some(items) = slot_item_lists.get(&s_str) {
            let d = items.iter().find(|it| it.origin == ItemOrigin::Equipped).unwrap_or(&items[0]);
...
pub fn is_valid_gear_set(gs: &HashMap<String, ResolvedItem>, spec: &str, catalyst: Option<u32>) -> bool {
...
pub fn is_baseline_gear_set(gs: &HashMap<String, ResolvedItem>) -> bool {
    GEAR_SLOTS.iter().all(|slot| gs.get(*slot).map_or(true, |i| i.origin == ItemOrigin::Equipped))
}
...
pub fn gear_set_identity_key(gs: &HashMap<String, ResolvedItem>) -> String {
    GEAR_SLOTS.iter().map(|slot| {
        if let Some(i) = gs.get(*slot) {
            ...
            format!("{}={}:{}:e{}:g{}", slot, i.item_id, b_key, i.enchant_id, i.gem_id)
        ...
        if let Some(max) = class_data::class_max_armor(class.as_str()) {
            for slot in ARMOR_SLOTS {
                ...
                if let Some(items) = slot_item_lists.get_mut(&s_str) {
                    items.retain(|i| {
                        if i.origin == ItemOrigin::Equipped { return true; }
                        if i.item_id == 0 { return true; }
                        game_data::get_item_armor_subclass(i.item_id).map_or(true, |s| s <= max || s == 0)
                    });
                }
            }
        }
...
pub struct UpgradeCombo { pub choices: Vec<(String, usize)> }
...
impl UpgradeDfsCtx<'_> {
    ...
    pub fn dfs(&mut self, idx: usize) {
        if idx == self.slots.len() {
            ...
            if total >= self.best_spend { self.retained.push(UpgradeCombo { choices: self.current.clone() }); }
            return;
        }
